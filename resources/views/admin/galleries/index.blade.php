...
@section('content')
    ...
    <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        @foreach ($galleries as $gallery)
            <div class="overflow-hidden rounded-3xl bg-white shadow-sm"><x-responsive-image src="{{ $gallery->image_path }}" alt="{{ $gallery->title }}" class="h-48 w-full object-cover" width="360" height="192" sizes="(min-width: 1024px) 33vw, 100vw" /><div class="p-5"><p class="text-xs font-bold uppercase tracking-[0.12em] text-gold">{{ $gallery->section }} / #{{ $gallery->sort_order }}</p><h3 class="mt-2 font-extrabold text-primary">{{ $gallery->title }}</h3><p class="mt-2 line-clamp-2 text-sm text-slate-500">{{ $gallery->description }}</p><div class="mt-4 flex gap-3"><a href="{{ route('admin.galleries.edit', $gallery) }}" class="font-bold text-primary">Edit</a><form method="POST" action="{{ route('admin.galleries.destroy', $gallery) }}" onsubmit="return confirm('Hapus galeri ini?')">@csrf @method('DELETE')<button class="font-bold text-red-600">Hapus</button></form></div></div></div>
        @endforeach
    </div>
