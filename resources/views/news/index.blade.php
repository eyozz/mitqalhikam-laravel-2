...
<head>
    ...
    <meta name="description" content="Berita, kegiatan, dan dokumentasi terbaru STTD Al Hikam Surakarta." />
    <meta property="og:title" content="News - MITQ Al Hikam Surakarta" />
    <meta property="og:description" content="Berita, kegiatan, dan dokumentasi terbaru STTD Al Hikam Surakarta." />
...
<body class="bg-background text-on-surface font-body-md overflow-x-hidden">
    <header class="fixed top-0 w-full flex justify-between items-center px-gutter py-4 bg-surface/95 backdrop-blur-sm z-50 border-b border-outline-variant/30 shadow-sm shadow-primary/5">
        <a href="{{ route('home') }}" class="flex items-center gap-sm"><img src="{{ asset('images/logo-al-hikam.svg') }}" alt="Logo STTD Al Hikam" class="w-12 h-12 object-cover rounded-full"><span class="font-semibold text-primary">MITQ AL-HIKAM</span></a>
        ...
        <a href="{{ config('services.ppdb_url', '#') }}" target="_blank" rel="noopener" class="bg-primary text-on-primary px-md py-sm rounded-lg text-xs font-semibold tracking-[0.1em] uppercase hover:bg-[#006747] transition-all">Daftar Sekarang</a>
    </header>
            ...
            <div class="max-w-container-max mx-auto grid lg:grid-cols-2 gap-lg items-center">
                ...
                @if ($featuredPost)
                    <a href="{{ route('news.show', $featuredPost) }}" class="group block overflow-hidden rounded-2xl border border-outline-variant bg-white shadow-xl">
                        <img src="{{ $featuredPost->image_url }}" alt="{{ $featuredPost->title }}" class="h-72 w-full object-cover transition duration-500 group-hover:scale-105">
                        <div class="p-md">
    ...
    <a href="{{ config('services.ppdb_url', '#') }}" target="_blank" rel="noopener" class="fixed bottom-6 right-6 z-50 rounded-full bg-gradient-to-r from-primary to-[#006747] px-md py-sm text-sm font-bold uppercase tracking-[0.08em] text-white shadow-2xl">Daftar PPDB</a>
</body>
</html>
